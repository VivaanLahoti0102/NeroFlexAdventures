...
# Function to create new obstacles (Tacos)
def create_obstacle():
    x = random.randint(50, SCREEN_WIDTH - 50 - OBSTACLE_WIDTH)
    y = -OBSTACLE_HEIGHT
    return {'rect': pygame.Rect(x, y, OBSTACLE_WIDTH, OBSTACLE_HEIGHT)}

# Function to draw the player with a "mouth"
def draw_player():
    pygame.draw.rect(screen, GREEN, (player_x, player_y, PLAYER_WIDTH, PLAYER_HEIGHT))
    # Draw head with a mouth
    head_radius = 15
    pygame.draw.circle(screen, (0, 0, 0), (player_x + PLAYER_WIDTH // 2, player_y - head_radius), head_radius)
    pygame.draw.rect(screen, RED, (player_x + PLAYER_WIDTH // 2 - 10, player_y - head_radius // 2, 20, 10))  # Mouth

# Function to draw obstacles (as tacos)
def draw_obstacles():
...
# Function to check for taco "eating"
def check_collision():
    global tacos_eaten, score
    mouth_rect = pygame.Rect(player_x + PLAYER_WIDTH // 2 - 10, player_y - 15, 20, 10)  # Mouth position
    for obstacle in obstacles:
        if mouth_rect.colliderect(obstacle['rect']):
            obstacles.remove(obstacle)
            tacos_eaten += 1
            if tacos_eaten % 5 == 0:  # Award 1 point for every 5 tacos eaten
                score += 1

# Function to process wrist movement
